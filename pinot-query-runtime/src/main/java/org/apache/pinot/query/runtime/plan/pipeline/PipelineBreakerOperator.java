...
class PipelineBreakerOperator extends MultiStageOperator {
  ...
  protected TransferableBlock getNextBlock() {
    ...
    // TODO (poc_yield): This is not the most effective implementation because nextBlock is blocking. Alternatives:
    //   - Use an ExecutorCompletionService and call nextBlock on a executor.
    //   - Add a concurrent queue between blocks, so we could call Operator.getResultQueue().take() to look for the
    //   first operator that actually offer a block.
    ...
    for (int i = 0; i < numWorkers; i++) {
      ...
      if (block != null) {
        ...
        // TODO (poc_yield): Can this actually not happen?
        //  It can be endOfStreamBlock if either:
        //    - it is an error, in which case it should have return in the previous if
        //    - if it is success, it should be catched by the previous if whose body is continue
